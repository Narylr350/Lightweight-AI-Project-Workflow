...
AI 在没有项目上下文时，会退回训练集默认值：标准 README、标准工程流程、标准表达风格。这个 pack 把使用者对项目的理解压缩进项目自身（`.ai/` 基线文件 + Git 交接），让无记忆或弱记忆的 AI 每次进入项目时，也能重新对齐项目的目标、边界、表达气质和维护习惯。

4 个 skill，Git 事实源，结构化 commit message，最小基线文件。
