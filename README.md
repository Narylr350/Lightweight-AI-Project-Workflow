...
一套面向个人项目和小型开源项目的 AI 项目工作流 skill。它要做的事很具体：让 AI 放大使用者对项目的判断，而不是用通用做法把它覆盖掉。

AI 在缺少项目上下文时会退回通用做法。通用做法本身没问题，但它不该盖过项目已经形成的目标、边界、表达方式和维护习惯——这些是使用者的选择，不是可以被平均值替换的默认项。

这个 pack 把这些选择保存成项目推进需要的工程上下文，尽量继承项目里已有的作者决定和具体锚点，让无记忆或弱记忆的 AI 在换线程、换模型后仍能对齐项目。未特别定义的部分继续用合适的默认做法。
