...
- 必读现有事实源：README、已有的 AI 指令文件（AGENTS.md / CLAUDE.md / AI_CONTEXT.md）、docs 下的 roadmap / overview / architecture、最近 commit、关键配置。
- 把现有文档里的**具体信息**提炼进基线：当前阶段、当前主线方向、优先级规则、真实约束、内容/数据流。不要把"5 条具体主线"压成一句"keep product usable"。
- 如果项目已有完整文档体系（如 docs/context、docs/tasks），.ai/PROJECT.md 不复述它们，而是提炼本 pack 工作流需要的关键信息，并在相应章节引用已有文档（如"详见 docs/context/development-roadmap.md"）。
- Goal / MVP / Seed Tasks 必须反映用户的**真实当前目标**。如果用户给的是维护目标（修 bug / 配置 / 文档），就把它落成具体维护方向，不要写成"这个项目是什么"的介绍。
...
- **不能写流程占位符**（如"确认第一个切片""创建任务记录""按需审计"）。这些是元任务，不是真实任务。Seed Task 要能直接交给 project-work 开工，比如"修复 X 配置漂移""排查 Y 功能是否可用"。
- 已有项目要从现有 roadmap / 用户真实目标提炼具体 Seed Tasks，不要写泛泛的流程步骤。
- 不能把未来想法写成当前任务。
