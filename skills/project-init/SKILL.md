...
- `.ai/TECH.md` — 技术方向，以及复杂项目必要的高层结构与边界
- `.ai/CONSTRAINTS.md` — 工作规则、硬约束、接入的 skill（执行层 + finish 层）
- `.ai/VALIDATION.md` — 验证命令和规则

这四个 `.ai/*.md` 基线文件默认是项目交接材料，应进 Git；`.ai/` 下的本地缓存、运行记录、日志、草稿、凭据或 `*.local.*` 不应进 Git。
...
project-init 不通过少量对话重新推导完整作者偏好。建立基线前按以下来源继承已有上下文：

1. **全局规范性指令** — 作者跨项目稳定的工作方式和判断规则。
2. **项目级规范性指令** — 当前仓库的定位、边界和修改规则。
3. **现有 `.ai/` 基线** — 已确认的项目目标和取舍。
4. **Git 与现有产物** — 项目实际采用并得到保留的实现和表达。
5. **当前对话** — 本轮新增或改变的明确决定。

越具体、越接近当前项目的明确决定优先。全局作者偏好是缺少具体信息时的默认参考，不能覆盖项目事实、项目级指令或用户当轮决定。全局和项目级指令不整份复制进 `.ai/`；只记录当前项目需要长期保留的差异和取舍。

**AI 指令与旧工作流先分类：** 文件名本身不能决定权威性。

- **仍生效的规范性作者 / 项目指令**（如当前有效的 AGENTS.md、CLAUDE.md）：继承，不作为普通历史材料降级。
- **旧任务状态和工作流产物**（如过期 docs/tasks、roadmap、plans、流水账）：进入 Workflow Authority Gate 判断保留、降级或替换。
- **与新基线冲突的规范性指令**：展示具体冲突，交用户决定优先级，不能静默覆盖。

**Workflow Authority Gate：** 发现旧工作流产物或权威源冲突时，必须明确选择处理方式并写入 `.ai/CONSTRAINTS.md`：
...
- 只看到项目里已有规则或旧文档内容，不等于 Workflow Authority Gate 已确认。发现旧 workflow 文件或权威源冲突时，必须在本轮 init 中向用户展示上述选项，或引用本轮用户已经明确给出的选择。
- `.ai/CONSTRAINTS.md` 只写当前仍有效的项目约束和工作规则，不复制"每次完成要更新某文档/某索引"这类已降级流程规范。
...
**Git Hygiene Gate：** 如果现有项目已能从源码、配置、README 或目录用途确认哪些生成物 / runtime / cache / logs / 下载物 / 本地凭据不应进 Git，把这些稳定规则写入 `.ai/CONSTRAINTS.md`。不能凭语言或框架模板脑补完整 `.gitignore`；无法确认的内容留给 project-work / project-finish 在实际出现时分类。

**表达上下文继承：** 优先从规范性指令和用户认可的现有 README、文档、代码或注释继承表达方式。只记录有文件证据或用户明确确认的项目表达取舍；少量对话、一次性纠偏和当前措辞不足以推导稳定作者偏好。必要时可在 `.ai/CONSTRAINTS.md` 引用 1-3 个已有文件、段落或实现作为参考锚点，并说明参考范围，不复制大段内容。没有明确锚点时不制造"朴素 / 正式 / 技术向"等人格标签；未特别定义的部分可以沿用项目现状或合适的通用做法。
...
如果项目已有部分 `.ai/` 基线，且 Git、现有文件和用户确认能证明其中内容仍有效，不重新跑完整初始化：

- 先列出缺失、过期和冲突项，保留仍有效的已确认内容。
- 只重新确认受影响的 gate；未受影响的 gate 继承现有已确认状态，不机械重复提问。
- 输出本轮基线变更摘要，明确修改项和保留不变项，不把整份旧基线重新生成一遍。
- 快速路径不豁免 gate；方向大改、权威源冲突、分支归属不明或 skill 接入变化时，仍必须重新确认对应 gate。

### 基线确认规则：整份确认，不默认通过

Stage C 必须显式确认整份 Baseline Decision Packet，不能只确认某一项就写入 `.ai/` 基线文件。Re-init 快速路径必须显式确认本轮基线变更摘要，并列明哪些现有基线内容保持不变。

- 禁止用"其余如无异议""默认同意""只确认技术方向"等措辞替代整份 baseline 确认。
- 如果使用 `request_user_input` / 用户输入面板，确认问题应针对整份 baseline 或本轮基线变更，例如"是否确认这份 baseline / 本轮变更写入 `.ai/` 基线文件？"
- 面板选项示例：
  - 确认写入
  - 需要调整范围 / MVP
  - 需要调整技术方向 / 约束
- 用户只确认某一项时（如只选了技术方向），只能视为该项确认，不能视为整份 baseline 确认。AI 必须再次就整份 baseline 征求确认。
- 用户确认整份 baseline 或本轮基线变更后，才可以写入 `.ai/` 基线文件。
- 用户确认 Baseline Decision Packet / 本轮基线变更摘要，只确认其中明确标为已确认的内容。不能把 Assumptions / Pending Questions / 待确认推荐通过整份确认洗成 confirmed baseline facts。
- 整份确认只覆盖 Tech Direction 中显式列出的技术选择点。藏在"实现原则"、Constraints、Non-goals 或普通 bullet 中的未论证技术选择，不能因整份确认自动变成已确认事实。
...
5. **Baseline Gate** — 确认 9 节 baseline（分布在 4 个文件中）；Re-init 快速路径确认本轮修改项和保留不变项。确保写入 `.ai/` 基线文件的都是已确认内容。
6. **Branch Scope Gate** — 确认本次 init / re-init 的 `.ai/` 基线属于项目主线、当前任务分支，还是暂不写入。防止在任务分支上重写主线基线。
7. **Workflow Authority Gate** — 确认旧任务 / 工作流产物如何处理，以及规范性指令与 `.ai/` 冲突时谁优先。防止旧流程和当前基线并存打架，也防止把仍生效的作者 / 项目指令误降级。
8. **Skill Selection Gate** — 确认执行层 / finish 层 skill 接入选择。防止把 AI 推荐的 skill 当成用户选择。

原先统称 Workflow Gate 的内容必须拆成 3 个 workflow-related gate：Branch Scope Gate、Workflow Authority Gate、Skill Selection Gate。只确认其中一个，不代表另外两个已经确认。
...
**其余不影响 `.ai/` baseline 的信息缺失才是非阻塞**：AI 可以作假设继续推进，在回复中展示 Assumptions，不连续追问。用户暂时不确定时，project-init 仍然可以继续生成 Baseline Decision Packet 或本轮基线变更摘要，但影响 baseline 的 gate 必须按"gate 不能跳过，不能降级"节处理。
...
1. 从 Goal / Users / I/O / Constraints 提取技术约束，并评估结构复杂度：职责数量、外部 I/O、持久化与状态、平台集成、测试 / 部署边界和长期迭代需求。
2. 输出 2-3 个候选方向，每个含：
   - **适用场景**：什么需求下它合适
   - **代价**：学习成本 / 依赖体积 / 维护负担
   - **不选它的原因**：什么情况下它是错的选择
   - **推荐**：AI 给一个推荐 + 理由（推荐不是决定）
3. 候选方向形成后，扫描其中隐含的**承重技术决定**：答案不同会影响多个模块、职责 / 依赖方向、状态所有权、持久化 / 通信 / 部署 / 测试方式，产生明显迁移成本，或导致后续实现做出不兼容选择。采用和拒绝框架、依赖、模式或结构都按同一标准判断。
4. 每个承重技术决定必须写明：**选择或推荐、需求 / 约束依据、主要替代、推荐理由**。依据不足时标为待确认推荐，必要时转成 Seed Task 调查，不能写入 `.ai/TECH.md`。
5. "轻量"、"MVP"、"个人项目"、"最少代码"、"避免过度工程化"只能参与取舍，不能单独推出"不使用框架 / 依赖 / 数据库 / 分层"等负向决定。
6. 明确列出需要用户确认的技术选择点。不能把承重技术决定藏在"实现原则"、Constraints、Non-goals 或普通 bullet 中随整份 baseline 顺带确认。
7. 当需求显示存在多个相对独立职责、多个外部边界、持久化 / 状态管理、平台集成，或明确需要长期迭代时，候选方向必须包含最小高层结构建议：主要层 / 模块、各自职责和依赖方向。结构选择属于 Tech Direction，用户确认后才写入 `.ai/TECH.md`。
8. 简单项目保持扁平，不为了显得完整强行分层。project-init 只记录维护所需的高层边界，不设计完整目录树、类层次或脚手架。
...
7. **Tech Direction** — 技术方向。含已确认的技术选择和理由；复杂项目另含已确认的高层分层 / 模块边界、职责和依赖方向。
...
- Baseline Decision Packet / 本轮基线变更摘要里必须区分：
