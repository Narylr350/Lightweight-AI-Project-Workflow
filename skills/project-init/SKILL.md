...
**表达气质识别：** 从现有 README、文档、代码注释中提炼项目的表达风格，写入 `.ai/CONSTRAINTS.md`。包括：文档语气（朴素 / 正式 / 技术向）、语言习惯（中英文混用比例、术语偏好）、代码注释风格（有无注释、详略）。已有项目按现有风格记录；新项目跟随用户对话中的表达习惯。后续 AI 写文档或注释时跟随这个气质，不自动套标准开源项目模板。
