...
3. 明确列出需要用户确认的技术选择点。
4. 当需求显示存在多个相对独立职责、多个外部边界、持久化 / 状态管理、平台集成，或明确需要长期迭代时，候选方向必须包含最小高层结构建议：主要层 / 模块、各自职责和依赖方向。结构选择属于 Tech Direction，用户确认后才写入 `.ai/TECH.md`。
5. 简单项目保持扁平，不为了显得完整强行分层。project-init 只记录维护所需的高层边界，不设计完整目录树、类层次或脚手架。
