...
本文件是给人看的设计背景文档，不是 AI 执行规则的载体。AI 需要执行的规则已写进各 SKILL.md 正文。本文件用于理解 skill 为什么这样设计，以及在设计新规则时参考。

---

## 核心

AI 是使用者能力的放大器，不是平均值生成器。

这个 pack 的所有规则都是这一条的下游。AI 可以承担劳动——写代码、跑验证、整理交接、补文档——但项目的方向、边界、表达方式和长期取舍属于使用者，AI 不接管这层意义。

对 AI 来说，项目现实不是仓库事实本身。Git、代码、文档只提供证据，说明项目发生过什么；这些事实意味着什么、哪里重要、哪里不能动，由使用者解释。缺少这层解释时，AI 会把仓库事实自动补成训练集里的平均项目——这正是这个 pack 要挡住的。

下面 14 条原则是这一条在具体场景里的展开。
...
轻量不是为了少做事，而是让 AI 的注意力停留在当前真实操作上：当前 Git 状态是什么，用户这次要什么，项目基线允许什么，本轮改了什么，怎么验证，下一步是否真的存在。

注意力不需要平均分配到项目的每个部分。作者可以决定哪些地方需要明确取舍，哪些地方可以采用成熟惯例和通用默认值。默认值不是问题；未经选择的默认值覆盖项目决定才是问题。

流程一重，注意力就会从"当前真实操作"转移到"完成流程表演"：写 spec、写 plan、补 design、维护 checklist、生成文档形式。看起来认真，但已经脱离真实任务。
...
- 尊重原项目工程风格：代码风格、目录结构、命名、技术栈、测试命令、文档语言和语气要跟随项目。改动已有产物文档（README、用户文档）的结构或表达方式属于影响项目气质的改动，先确认；小修（修错别字、更新命令、加一行说明）不需要确认。**但这不覆盖本 pack 的工作流契约**：`.ai/` 基线文件、Why/What/Verify/Next/Notes、Git 核验、Decision Ownership、finish/audit 规则仍然生效。
...
未完成工作因额度、上下文或工具故障被迫中断时，可以由用户手动触发 project-handoff，生成 `.ai/HANDOFF.local.md`。它是 Git 外的临时运输材料，不是第四层事实，不属于基线，也不能替代 commit message；下一位 AI 仍要从事实层重新核验。

作者与项目上下文按具体程度继承：全局作者指令 → 项目规范性指令 → `.ai/` 项目基线 → Git 与已有产物 → 当前对话。越接近当前项目的明确决定优先。全局偏好是缺少具体信息时的默认参考，不能覆盖项目事实或用户当轮决定。

这套继承只能尽量减少上下文损失，不替代长期对话记忆，也不从少量对话推导完整作者画像。优先保存已有的具体锚点和已确认取舍，而不是制造抽象人格标签。
...
- `.ai/HANDOFF.local.md` 是用户手动触发的临时接力，不是基线文档，不进入 Git。
- 已提交工作的日常进度由 commit message 承载；未完成工作的 handoff 只在不可正常收尾时临时使用。
...
任何交接信息（commit Next / 对话 summary / `.ai/HANDOFF.local.md`）都可能：
...
临时 handoff 只保存与当前未完成工作有关的增量：当前任务、进度、已确认决策、未完成状态、验证边界、失败路径、运行时状态和最近下一步。已有基线、完整 diff、长日志和完整对话只引用，不重复搬运。handoff 不能自动触发；是否生成由用户决定。
...
- `project-handoff` — 用户手动触发的未完成工作接力
- `project-audit` — 低频复盘

用户不需要记内部阶段。project-work 内部判断任务类型，但不暴露为命令。不新增 intake / execute / closeout 等额外常态入口。project-handoff 是不可正常收尾时的手动异常入口，不进入常态循环，也不能由 AI 自行触发。
...
项目产物（代码、README、用户文档、产品文案）写项目的事，不写 AI 的事。
...
产物层只写项目内容：项目做什么、怎么用、怎么装、怎么测。用户纠偏原话、AI 失败过程、AI 自我警告属于交接层（commit Notes），不属于产物层。

README 是对外产物，面向 GitHub 访客和项目使用者。写项目是什么、能干什么、怎么装、怎么用。不写当前开发状态、迁移方向、内部工作流规则--这些属于 `.ai/` 基线文件。不要把 README 当成 `.ai/` 的一部分。

内部设计讨论不进对外产物。README 是给用户和开发者看的，不是设计辩论记录。
...
**识别时机：** init / re-init 时识别项目**已有**的维护表面，不强制复杂模板，**不创造不存在的维护表面**。维护表面包括：
...
项目没有 CHANGELOG，不创建。没有 `.env.example`，不创建。Maintenance Surface 只记录项目实际有的文件，供 finish 触发式检查用。被 Workflow Authority Gate 降级为历史参考的文件不属于 Maintenance Surface，不维护、不同步。
